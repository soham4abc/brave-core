...
class BATLedgerContext {
 public:
  ...
  // Components define a static |kComponentKey| member. Instances are
  ...
  //   };
  //
  // Jobs must implement a public |future()| method that returns an Future
 ...
 private:
  ...
  std::map<size_t, std::unique_ptr<Object>> components_;
  std::map<void*, std::unique_ptr<Object>> jobs_;
