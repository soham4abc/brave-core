...
SQLStore::CommandFuture SQLStore::Open(int current_version) {
...
SQLStore::CommandFuture SQLStore::Close() {
  auto command = mojom::DBCommand::New();
  command->type = mojom::DBCommand::Type::CLOSE;
  return RunTransaction(std::move(command));
}

SQLStore::CommandFuture SQLStore::RunMigration(
    int version,
...
SQLStore::CommandFuture SQLStore::Vacuum() {
  auto command = mojom::DBCommand::New();
...
SQLStore::CommandFuture SQLStore::RunTransaction(mojom::DBCommandPtr command) {
  auto transaction = mojom::DBTransaction::New();
  transaction->commands.push_back(std::move(command));
  return RunTransactionImpl(std::move(transaction));
}

SQLStore::CommandFuture SQLStore::RunTransactionImpl(
    mojom::DBTransactionPtr transaction) {
  DCHECK(transaction);

  CommandFuture::Resolver resolver;
