...
const size_t URLFetcher::kComponentKey =
    BATLedgerContext::ReserveComponentKey();

URLFetcher::URLFetcher() = default;
URLFetcher::~URLFetcher() = default;

Future<mojom::UrlResponsePtr> URLFetcher::Fetch(mojom::UrlRequestPtr request) {
...
Future<mojom::UrlResponsePtr> URLFetcher::FetchImpl(
    mojom::UrlRequestPtr request,
    FetchOptions options) {
  DCHECK(request);
  LogURLRequest(*request, options);

  Future<mojom::UrlResponsePtr>::Resolver resolver;
